...
//가상의 데이터(마운트 : 서버에서 데이터를 가져온다. (Ajax로 json파일을 가져온다.))
const mockData = [
  {
    id: 0,
    isDone: false,
...
function reducer(state,action) {
  switch(action.type){
    ...
    case "UPDATE": return state.map((item)=>{ return item.id === action.data ? {...item, isDone: !item.isDone} : item });
...
function App() {
  //const [todos, dispatch] = useState(mockData);
