...
//     //2초동안 작업처리한 것임.
//     //executor 처리문문
//     //숫자값을 주면 타입을 점검해서 number타입이면 resolve, 아니면 reject
//      setTimeout(()=>{
//         const num = "십";
...
//5. promise 실제적용 성공했을 때와 실패했을 때를 처리한다.
...
//     });
    ...
    //6. promise 실제적용 성공했을 때와 실패했을 때를 처리한다. => 편리하게 처리
    //promise 체인
    // promise5.then((value)=>{
    //     console.log(value);
    // }).catch((value)=>{
    ...
    //음식을 주문하는 상황황 promise chain 진행해보자
    ...
    function coolFood2(flag, food){
        const promise = new Promise((resolve,reject)=>{
            console.log(food+"음식 차게 주문");
            setTimeout(()=>{
                //let flag = true;
                if(flag === true) {
                    resolve(food+"음식 차게 완료");
                }else {
                    resolve(food+"음식 차게 실패");
                }
            },3000);
        });
        return promise;
